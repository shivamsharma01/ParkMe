...
public interface ParkMeDAO {
    ...
    @Query("UPDATE query_table SET status =:status AND close_time =:closeTime where qid=:qid")
    void updateCancelRequest(String status, long closeTime, int qid);

    @Query("UPDATE query_table SET status =:status AND close_time =:closeTime AND rating =:rating where qid=:qid")
